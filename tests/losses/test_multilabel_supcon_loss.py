...
class TestMultiSupConLoss(unittest.TestCase):
    def test_multi_supcon_loss(self):
        ...
        embeddings = torch.tensor([[0.1, 0.3, 0.1],
                                ...
                                [0.1, -0.16, 0.1],
                                [0.13, -0.13, 0.2]])
        labels = torch.tensor([[1,0,1], [1,0,0], [0,1,1], [0,1,0]])
        affine_net = torch.nn.Linear(3, 3, bias=False)
        affine_net.weight.data = torch.ones_like(affine_net.weight.data)
        embeddings = affine_net(embeddings)
        loss = loss_func(embeddings, labels)
        self.assertTrue(loss>0)
